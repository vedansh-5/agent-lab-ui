...
async def try_local_diagnostic_run(
        ...
        adk_user_id: str,
        message_text: str,
        # Pass explicitly required env vars instead of relying on global os.environ manipulation within this function
        # This makes the function more testable and dependencies clearer.
        # The caller (orchestrator) should prepare these.
    ...
    if a remote query yielded no results or errors.

    Returns:
        A list of diagnostic error messages. Empty if local run was successful or skipped.
    """
    logger.warn(f"[LocalDiag] Initiating local diagnostic run for Firestore Agent ID: '{firestore_agent_id}'.")
    diagnostic_errors = []
    agent_config_data = None
    original_env = {} # To store original values of env vars we might temporarily set

    try:
        # Store and temporarily set environment variables for LiteLLM within diagnostic scope
        # This is tricky because os.environ is global. Better to pass to LiteLlm if possible.
        # For now, we'll manage it carefully.
        vars_to_manage = ["GOOGLE_CLOUD_PROJECT", "GOOGLE_CLOUD_LOCATION"] # Add others if LiteLlm needs them
        if project_id_for_diag: vars_to_manage.append("GOOGLE_API_KEY") # Example, if LiteLlm uses this for Gemini
        ...
        if location_for_diag and os.environ.get("GOOGLE_CLOUD_LOCATION") != location_for_diag:
            ...
            # Note: API keys for various providers (OpenAI, Anthropic, etc.) should already be set
        # in the Firebase Function's environment if the agent config uses them.
        # We are not overriding those here, just ensuring GCP context vars are set if needed by a provider.
        ...
        logger.debug(f"[LocalDiag] Agent config dump: {str(agent_config_data)[:500]}...") # Log part of config

        # instantiate_adk_agent_from_config handles LiteLlm model instantiation based on config
        local_adk_agent = instantiate_adk_agent_from_config(
            agent_config_data,
        ...
        local_artifact_service = InMemoryArtifactService() # Required by Runner
        local_memory_service = InMemoryMemoryService()     # Required by Runner

        local_runner = Runner(
            agent=local_adk_agent,
            app_name=agent_config_data.get('name', f"local_diag_app_{firestore_agent_id[:4]}"), # Use sanitized name
        ...
        async for diag_event_obj in local_runner.run_async(user_id=adk_user_id, session_id=local_session.id, new_message=local_message_content):
            ...
            # Check for text in the event to build a response, similar to query_vertex_runner
        ...
        if local_events_count == 0 and not local_final_response_text:
            diagnostic_errors.append(f"[LocalDiag] Local diagnostic run for '{config_name_for_log}' produced NO events and NO text. This might indicate an issue in the agent's core logic or tool setup that doesn't throw exceptions but also doesn't yield output. Check agent instructions and tool interactions.")
            logger.warn(diagnostic_errors[-1])
        elif not local_final_response_text: # Has events but no text
            diagnostic_errors.append(f"[LocalDiag] Local diagnostic run for '{config_name_for_log}' produced {local_events_count} events but NO final text response. Check agent's output handling and if it's correctly forming text parts in events.")
            logger.warn(diagnostic_errors[-1])
        else: # Has events and text, which means it worked locally.
            diagnostic_errors.append(f"[LocalDiag] Agent '{config_name_for_log}' ran locally without raising an ADK-level exception and produced text: '{local_final_response_text[:100]}...'. The issue with the remote query is likely specific to the Vertex AI environment, deployment packaging, service permissions, or a subtle difference not caught by local LiteLLM execution.")
            logger.info(diagnostic_errors[-1])
        ...
        logger.error(f"{err_msg}\n{traceback.format_exc()}")
        # Include a snippet of traceback for easier debugging from client-side error display
        tb_lines = traceback.format_exc().splitlines()
        relevant_tb = "\n".join(tb_lines[-5:]) # Last 5 lines of traceback
        ...
        for var_name, orig_value in original_env.items():
            if orig_value is None:
                ...
                if os.environ.get(var_name) != orig_value:
